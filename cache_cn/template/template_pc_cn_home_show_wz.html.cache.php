...
<body>
    ...
    <div class="gjgwgog w85">
        ...
        <p class="nr"><?php echo $description; ?></p>
        <div class="contentss">
            <div class="rightsff">
                ...
                <ul class="xg">
                    <?php if ($xgzl) { ?>
                    <?php $list_return = $this->list_tag("action=module module=news IN_id=$xgzl catid=1 more=1 order=displayorder num=5"); if ($list_return) extract($list_return, EXTR_OVERWRITE); $count=count($return); if (is_array($return)) { foreach ($return as $key=>$t) { $is_first=$key==0 ? 1 : 0;$is_last=$count==$key+1 ? 1 : 0; ?>
                    <li><a href="<?php echo $t['url']; ?>"><?php echo $t['title']; ?></a></li>
                    <?php } } ?>
                    <?php } ?>
                </ul>
