...
// 安装程序
class Install extends \Soulcms\Common
{
    ...
    public function __construct(...$params)
    {
        ...
        is_file($this->lock) && exit('安装程序已经被锁定，重新安装请删除：cache_'.DIY_LANG.'/install.lock');
        ...
        switch ($step) {
            ...
            case 2:
                ...
                if (IS_AJAX_POST) {
                    ...
                    // 存储缓存文件中
                    $size = @file_put_contents(WRITEPATH.'install.info', mys_array2string($data));
                    if (!$size || $size < 10) {
...
//     \'DBPrefix\'	=> \''.mys_safe_filename($data['db_prefix']).'\',
// ];';
...
//                     }
            ...
            case 3:
                ...
                $error = '';
                $data = mys_string2array(file_get_contents(WRITEPATH.'install.info'));
                ...
                if (empty($data)) {
                    ...
                    if (!$this->db->connect(false)) {
                        ...
                        if (strlen($errorlog) > 10) {
                            ...
                            \Soulcms\Service::M('System')->save_config($sys, $sys);
        ...
        \Soulcms\Service::V()->display('install/'.$step.'.html');
        exit;
    }
