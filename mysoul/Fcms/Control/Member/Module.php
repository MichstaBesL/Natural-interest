...
// 内容模块操作类 基于 Ftable
class Module extends \Soulcms\Table
{
    ...
    public function __construct(...$params) {
        ...
        // 模块显示名称
        $this->name = mys_lang('内容模块[%s]（%s）', $this->module['dirname'], $this->module['title']);
        // 初始化数据表
        $this->_init([
    ...
    protected function _Member_Add() {
        ...
        $id = 0;
        $did = intval(\Soulcms\Service::L('input')->get('did'));
        $catid = intval(\Soulcms\Service::L('input')->get('catid'));
        ...
        if (!$this->is_hcategory) {
            ...
            if (!$catid) {
                list($select, $catid) = \Soulcms\Service::L('Tree')->select_category(
                    $category,
                    $catid,
                    'id=\'mys_catid\' name=\'catid\' onChange="show_category_field(this.value)"',
                    '', 1, 1, 1
                );
            } else {
            ...
            !$category[$catid] && exit($this->_msg(0, mys_lang('当前栏目(%s)没有发布权限', (int)$catid)));
            $this->is_post_code = mys_member_auth($this->member_authid, $this->member_cache['auth_module'][SITE_ID][$this->module['dirname']]['category'][$catid]['code']);
        } else {
            // 不走栏目权限，走自定义权限
        ...
        \Soulcms\Service::V()->assign([
            'did' => $did,
            'mcid' => 'add',
            'form' =>  mys_form_hidden(['is_draft' => 0, 'module' => $this->module['dirname'], 'id' => $id]),
            'select' => $select,
            'draft_url' =>\Soulcms\Service::L('Router')->member_url($this->module['dirname'].'/home/add'),
            'draft_list' => $this->content_model->get_draft_list('cid='.$id),
            'is_post_code' =>  $this->is_post_code,
            'category_field_url' =>  $this->is_post_code || $this->module['category_data_field'] ? \Soulcms\Service::L('Router')->member_url($this->module['dirname'].'/home/add') : ''
        ]);
        \Soulcms\Service::V()->display($tpl);
    }
    ...
    protected function _Member_Edit() {
        ...
        $this->is_get_catid = intval(\Soulcms\Service::L('input')->get('catid'));
        ...
        if (!$data) {
            $this->_msg(0, mys_lang('内容不存在'));
            exit;
        } elseif ($this->uid != $data['uid']) {
            $this->_msg(0, mys_lang('无权限操作'));
            exit;
        } elseif (defined('IS_MODULE_VERIFY') && $data['status'] != 0) {
            // 判断是否来至审核
            $this->_msg(2, mys_lang('正在审核之中'));
            exit;
        }
        ...
        if (!$this->is_hcategory) {
            ...
            !$category[$data['catid']] && exit($this->_msg(0, mys_lang('当前栏目[%s]没有修改权限', "#" . $data['catid'])));
        } else {
        ...
        \Soulcms\Service::V()->assign([
            ...
            'draft_url' =>\Soulcms\Service::L('Router')->member_url($this->module['dirname'].'/home/edit', ['id' => $id]),
            'draft_list' => $this->content_model->get_draft_list('cid='.$id),
            'is_post_code' => $this->is_hcategory ? $this->content_model->_hcategory_member_post_code() : $this->is_post_code,
            'category_field_url' => $this->is_post_code || $this->module['category_data_field'] ? \Soulcms\Service::L('Router')->member_url($this->module['dirname'].'/home/edit', ['id' => $id]) : ''
        ]);
        \Soulcms\Service::V()->display($tpl);
    }
    ...
    protected function _Member_Del() {
        ...
        if (!$id) {
            $this->_json(0, mys_lang('id不存在'));
        }

        $row = \Soulcms\Service::M()->table(SITE_ID.'_'.$this->module['dirname'].'_index')->get($id);
        if (!$row) {
            $this->_json(0, mys_lang('内容不存在'));
        } elseif ($row['uid'] != $this->uid) {
            $this->_json(0, mys_lang('不能删除别人的内容'));
        }

        if (!$this->is_hcategory) {
            $cat = $this->_module_member_category($this->module['category'], $this->module['dirname'], 'del');
            !isset($cat[$row['catid']]) && $this->_json(0, mys_lang('当前栏目没有删除权限'));
        } else {
            $this->content_model->_hcategory_member_del_auth();
        }

        $rt = $this->content_model->delete_to_recycle([$id]);
        $rt['code'] ? $this->_json(1, mys_lang('删除成功')) : $this->_json(0, $rt['msg']);
    }
    ...
    protected function _Member_Verify_List() {
        ...
        \Soulcms\Service::V()->display('module_verify.html');
    }
    ...
    protected function _Member_Verify_Del() {
        ...
        $this->_Del(\Soulcms\Service::L('input')->get_post_ids(), null, function($rows) {
            foreach ($rows as $t) {
                // 删除索引
                $t['isnew'] && \Soulcms\Service::M()->table(SITE_ID.'_'.$this->module['dirname'].'_index')->delete($t['id']);
            }
    ...
    protected function _Member_Draft_List() {
        ...
        \Soulcms\Service::V()->display('module_draft.html');
    }
    ...
    public function syncat_edit() {
        ...
        $sync = \Soulcms\Service::L('input')->get('catid');

        if (IS_AJAX_POST) {

            $catid = \Soulcms\Service::L('input')->post('catid');
            !$catid && $this->_json(0, mys_lang('你没有选择同步的栏目'));

            $syncat = [];
            foreach ($catid as $i) {
                if ($this->where_list_sql && \Soulcms\Service::M('content', 'cqx')->is_edit($i)) {
                    $this->_json(0, mys_lang('当前角色无权限管理此栏目'));
                }
            ...
            !$syncat && $this->_json(0, mys_lang('所选栏目无效'));

            $this->_json(1, mys_count($syncat), implode(',', $syncat));
        }
        ...
        \Soulcms\Service::V()->display('share_syncat.html');exit;
    ...
    protected function _Data($id = 0) {
        ...
        // 判断是否来至审核
        if (defined('IS_MODULE_VERIFY')) {
            $row = \Soulcms\Service::M()->table(SITE_ID.'_'.$this->module['dirname'].'_verify')->get($id);
            if (!$row) {
                return [];
            ...
            $data['backinfo'] = mys_string2array($row['backinfo']);
            !$this->is_get_catid && $this->is_get_catid = (int)$row['catid'];
            // 栏目验证码
            $this->is_post_code = $this->is_hcategory ? $this->content_model->_hcategory_member_post_code() : mys_member_auth($this->member_authid, $this->member_cache['auth_module'][SITE_ID][$this->module['dirname']]['category'][$this->is_get_catid]['code']);
            return $data;
        }
    ...
    protected function _Format_Data($id, $data, $old) {
        ...
        // 验证栏目   验证码
        $catid = (int)\Soulcms\Service::L('input')->post('catid');
        if (!$this->module['category'][$catid] && !$this->is_hcategory) {
            $this->_json(0, mys_lang('栏目[%s]不存在', $catid), ['field' => 'catid']);
        }
    ...
    protected function _Save($id = 0, $data = [], $old = [], $func = null, $func2 = null) {
        ...
        $did = intval(\Soulcms\Service::L('input')->get('did'));
        $is_draft = intval(\Soulcms\Service::L('input')->post('is_draft'));
            ...
            return parent::_Save($id, $data, $old,
                ...
                function ($id, $data, $old) {
                    ...
                    if ($old) {
                        ...
                        if (!$cat[$data[1]['catid']]) {
                            return mys_return_data(0, mys_lang('当前栏目(%s)没有发布权限', (int)$data[1]['catid']));
                        }
                        // 判断日发布量
                        ...
                        if ($this->uid && $day_post
                            ...
                            return mys_return_data(0, mys_lang('当前栏目每天发布数量不能超过%s个', $day_post));
                        }
                        // 判断发布总量
                        ...
                        if ($this->uid && $total_post
                            && \Soulcms\Service::M()->db
                                    ...
                                    ->where('catid', $data[1]['catid'])
                                    ->countAllResults() >= $day_post) {
                            return mys_return_data(0, mys_lang('当前栏目的发布数量不能超过%s个', $total_post));
                        }
                        // 金币验证
                        ...
                        if ($this->uid && $score + $this->member['score'] < 0) {
                            return mys_return_data(0, mys_lang(SITE_SCORE.'不足，本次需要%s'.SITE_SCORE, abs($score)));
                        }
                    }
                    return mys_return_data(1, '', $data);
                },
                ...
                function ($id, $data, $old) {
                    // 同步发送到其他栏目
                    if ($data[1]['status'] == 9 && \Soulcms\Service::L('input')->post('sync_cat')) {
                        $this->content_model->sync_cat(\Soulcms\Service::L('input')->post('sync_cat'), $data);
                    }
                    return $data;
                }
            );
    ...
    protected function _Call_Post($data) {
        ...
        if ($data[1]['status'] == 9) {
            ...
            if ($this->module['category'][$data[1]['catid']]['setting']['html']) {
                // 生成权限文件
                !mys_html_auth(1) && $this->_json(0, mys_lang('/cache/html/ 无法写入文件'));
            ...
            $this->_json(1, mys_lang('操作成功'), ['id' => $data[1]['id'], 'catid' => $data[1]['catid'], 'htmlfile' => $html, 'listfile' => $list]);
        } else {
            \Soulcms\Service::L('input')->post('is_draft') ? $this->_json(1, mys_lang('操作成功，已存储到草稿箱')) : $this->_json(1, mys_lang('操作成功，等待管理员审核'), ['id' => $data[1]['id'], 'catid' => $data[1]['catid']]);
        }
    }
}
